...
let private toolsJson = """[
  {"name":"debug_open","description":"Open a verifrog project for debugging. Required before using other debug tools. Pass the path to the directory containing verifrog.toml (e.g., '/path/to/myproject' or '/path/to/myproject/verifrog.toml'). To switch projects, call again with a different path — the previous sim is closed and a new one loaded.","inputSchema":{"type":"object","properties":{"project_path":{"type":"string","description":"Absolute path to the project directory containing verifrog.toml, or direct path to verifrog.toml itself"}},"required":["project_path"]}},
